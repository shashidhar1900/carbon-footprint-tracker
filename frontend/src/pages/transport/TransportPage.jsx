...
export function TransportPage() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const response = await api.get('/transport-service/api/transport/history');
  ...
  const handleAddOrUpdate = async (event, isUpdate) => {
    ...
    try {
      const payload = { mode: formData.mode, distance: Number(formData.distance) };
      if (isUpdate) {
  ...
  const handleDeleteConfirmed = async () => {
    ...
    try {
      await api.delete(`/transport-service/api/transport/delete/${deleteTarget}`);
        ...
        <form className="service-form">
          <div className="service-form-row">
            <div>
              ...
              <select id="mode" name="mode" value={formData.mode} onChange={handleFormChange}>
                {MODES.map((mode) => (
          ...
          <div className="service-form-actions">
            ...
            <button
              ...
              title={!todayEntryExists ? 'No entry for this mode today yet' : undefined}
            >
              Update entry
            </button>
            <button
                  ...
                  <td className="service-table-actions">
                    {row.date === TODAY ? (
                      <>
                        ...
                        <button type="button" className="danger-link" onClick={() => confirmDelete(row.mode)}>
                          Delete
                        </button>
                      </>
                    ) : (
      ...
      {deleteTarget && (
        <div className="modal-overlay" role="dialog" aria-modal="true">
          <div className="modal-card">
            ...
            <p>This removes all logged distance for this mode. This can&apos;t be undone.</p>
